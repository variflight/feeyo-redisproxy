 ...
 *
 * @see com.feeyo.net.codec.redis.RedisRequestDecoder
 */
public class RedisRequestDecoderV2 implements Decoder<List<RedisRequest>> {
    ...
    public List<RedisRequest> decode(byte[] buffer) throws UnknowProtocolException {
        ...
        try {
            ...
            int byteLength = byteArray.getByteCount();
            decode:
            for (; ; ) {
                switch (state) {
                    case READ_SKIP: {
                    ...
                    case READ_INIT: {
                        if (readOffset >= byteLength || (argCount != 0 && argCount == argIndex + 1)) {
                    ...
                    case READ_END: {
                        // 处理粘包
                        if (byteLength < readOffset) {
                            throw new IndexOutOfBoundsException("Not enough data.");
                        } else if (byteLength == readOffset) {
