...
public class SlowProcKeyColletor implements StatCollector {
	...
	private static final long SLOW_COST = -1L; //指令处理时间
	...
	private static long calculateThresholdCount(Map<String, SlowProcKeyInfo> slowKeyMap) {
		return (long) (calculateAvgCount(slowKeyMap) * WEED_OUT_RADIO) ;
	}
	
	private static long calculateAvgCount(Map<String, SlowProcKeyInfo> slowKeyMap) {
		long count_sum = 0;
	...
	public static Map<String, SlowProcKeyInfo> getAscSlowKeyMap(Map<String, SlowProcKeyInfo> slowKeyMap ){
		ascSortedMap.clear();
