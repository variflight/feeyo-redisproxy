 ...
 *
 */
public class BypassThreadExecutor extends ThreadPoolExecutor {
	...
	public BypassThreadExecutor() {
		this(DEFAULT_MIN_THREADS, DEFAULT_MAX_THREADS);
	}

	public BypassThreadExecutor(int coreThread, int maxThreads) {
		this(coreThread, maxThreads, maxThreads);
	}

	public BypassThreadExecutor(int coreThread, int maxThreads, long keepAliveTime, TimeUnit unit) {
		this(coreThread, maxThreads, keepAliveTime, unit, maxThreads);
	}

	public BypassThreadExecutor(int coreThreads, int maxThreads, int queueCapacity) {
		this(coreThreads, maxThreads, queueCapacity, Executors.defaultThreadFactory());
	}

	public BypassThreadExecutor(int coreThreads, int maxThreads, int queueCapacity, ThreadFactory threadFactory) {
		this(coreThreads, maxThreads, DEFAULT_MAX_IDLE_TIME, TimeUnit.MILLISECONDS, queueCapacity, threadFactory);
	}

	public BypassThreadExecutor(int coreThreads, int maxThreads, long keepAliveTime, TimeUnit unit, int queueCapacity) {
		this(coreThreads, maxThreads, keepAliveTime, unit, queueCapacity, Executors.defaultThreadFactory());
	}

	public BypassThreadExecutor(int coreThreads, int maxThreads, long keepAliveTime, TimeUnit unit,
	...
	public void execute(Runnable command) {
		...
		// 超过最大的并发任务限制，进行 reject 依赖的LinkedTransferQueue没有长度限制，因此这里进行控制 
...
class ExecutorQueue extends LinkedTransferQueue<Runnable> {
	...
	BypassThreadExecutor threadPoolExecutor;

	public ExecutorQueue() {
		super();
	}

	public void setStandardThreadExecutor(BypassThreadExecutor threadPoolExecutor) {
